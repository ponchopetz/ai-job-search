...
- **Job portals:** "This fork includes job search tools for Denmark (Jobindex, Jobnet, Akademikernes Jobbank, Jobdanmark), a country-agnostic LinkedIn tool, and two US tools — Adzuna (needs a free API key from developer.adzuna.com) and RemoteOK (no signup needed). Are any of these useful for you, or do you search other sites?" Note: if none of the shipped tools fit the user's market, point them at `/add-portal` to generate an equivalent CLI integration for their local job board, or suggest relying on LinkedIn/Google site-searches in the meantime.
